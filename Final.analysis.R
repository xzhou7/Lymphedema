...
data <- read.csv("./Co-morbidities.csv", header = T, row.names = 1)
...
pindi <- ggplot(databind2, aes(x=Axis.1, y=Axis.2, color=Condition, shape=as.factor(Obesity))) + geom_point(na.rm = T)
pindi <- pindi + scale_color_jama() + theme(legend.position = "none")
pindi

pindi2 <- ggplot(databind2, aes(x=Axis.3, y=Axis.4,color=Condition, shape=as.factor(Obesity))) + geom_point(na.rm = T)
pindi2 <- pindi2 + scale_color_jama()
pindi2

pindi3 <- ggplot(databind2, aes(x=Axis.5, y=Axis.6,color=Condition, shape=as.factor(Obesity))) + geom_point(na.rm = T)
...
row.names(databind2.1) <- paste("N", 1:715, sep="_")

pindi.all <- ggplot(databind2.1, aes(x=Axis.1, y=Axis.2, color=Condition, shape=as.factor(Obesity))) + geom_point(na.rm = T)
pindi.all <- pindi.all + scale_color_jama() + theme(legend.position = "none")
pindi.all

pindi2.all <- ggplot(databind2.1, aes(x=Axis.3, y=Axis.4,color=Condition, shape=as.factor(Obesity))) + geom_point(na.rm = T)
pindi2.all <- pindi2.all + scale_color_jama()
pindi2.all

pindi3.all <- ggplot(databind2.1, aes(x=Axis.5, y=Axis.6,color=Condition, shape=as.factor(Obesity))) + geom_point(na.rm = T)
...
#"#000000", "#555555" /// "#ed553b", "#e98300"
pclust <- fviz_dend(hc2, cex = 1, k=1 ,type = "phylogenic", rect = TRUE, repel=T ) + scale_color_manual(values=c("#000000"))
pclust <- pclust + ggtitle("Disease relationship of patient with lymphatic disease (n=616)")
pclust
ggsave(file = "./figure/Figure4_black.pdf",pclust, height = 8, width = 12, dpi=300)

write.csv(file= "./Disease_pairwised_distance.csv", as.matrix(Disease.dist))
...
#000000 #175e54
pclust2 <- fviz_dend(hc3, cex = 1, k=1 ,type = "phylogenic", rect = TRUE, repel=T, k_colors = "red") + scale_color_manual(values=c("red"))
pclust2 <- pclust2 + ggtitle("Disease relationship of patient without lymphatic disease (n=99)")
pclust2
ggsave(file = "./figure/Figure4NM_red.pdf",pclust2, height = 9, width = 7.5, dpi=300)

write.csv(file= "./Normal_pairwised_distance.csv", as.matrix(NM.dist))
#set up mantel test
...
###added on Nov. 15#########################################
#only secondary lymphedema and control
data_SeCo <- subset(data_ori, Condition %in% c("NM","SL"))
data_SeCo<- data_SeCo[,21:144]
data_SeCo[is.na(data_SeCo)] <- 0
data_SeCo2 <- data_SeCo[!(rowSums(data_SeCo)==0),!(colSums(data_SeCo)==0)]

SeCo2.dist1 <- vegdist(as.data.frame(t(data_SeCo2)), "jaccard",na.rm = T, binary = T)
SeCo2.dist <- as.data.frame(as.matrix(SeCo2.dist1))
PCOA.dist_SeCo2 <- pcoa(SeCo2.dist)
pengi.nm<-barplot(PCOA.dist_SeCo2$values$Relative_eig[1:10])
biplot.pcoa(PCOA.dist_SeCo2)
dim(PCOA.dist_SeCo2$vectors[,1:20])

df.SeCo2<- as.data.frame(PCOA.dist_SeCo2$vectors[,1:20])
df.SeCo2$Disease <- colnames(data_SeCo2)
df.SeCo2$Type <- sub("(_[^_]+)$", "",df.SeCo2$Disease)

p.SeCo2 <- ggplot(df.SeCo2, aes(x=Axis.1, y=Axis.2, color=Type, label=Disease)) + geom_point() + geom_text_repel(data=subset(df.disease, Type=="LYMP"))
p.SeCo2 

hc.SeCo2 <- hclust(SeCo2.dist1, method = "ward.D")
plot(hc.SeCo2, cex = 0.8, hang = -1)

pclust.SeCo2 <- fviz_dend(hc.SeCo2, cex = 1, k=1 ,type = "phylogenic", rect = TRUE, repel=T, k_colors = "black") + scale_color_manual(values=c("black"))
pclust.SeCo2 <- pclust.SeCo2 + ggtitle("Disease relationship of patient with secondary Lymphodema and control (n=506)")
pclust.SeCo2
ggsave(file = "./figure/Figure4_SLandNM_Black.pdf", pclust.SeCo2, height =9, width = 7.5, dpi=300)

write.csv(file= "./Normal_SL_pairwised_distance.csv", as.matrix(SeCo2.dist1))


###added on Jan.14 #########################################
#remove cancer patient in secondary lymphedema patient
data_SL <- subset(data_ori, Condition %in% c("SL"))
data_SL<- data_SL[,21:144]
colnames(data_SL)

Cancerlist <- c("Breast.cancer","Cervical.Cancer" ,"Prostate.cancer", "Endometrial.cancer", "Colon.cancer", "Lymphoma", "Melanoma", "Basal.cell.carcinoma","Squamous.cell.carcinoma")

data_SL_nocancer <- data_SL[which(! colnames(data_SL) %in% Cancerlist)]

data_cancer <- data_SL[which(colnames(data_SL) %in% Cancerlist)]
colSums(data_cancer)

data_cancer1 <- data_no_NM[which(colnames(data_no_NM) %in% Cancerlist)]
colSums(data_cancer1)


data_SL_nocancer[is.na(data_SL_nocancer)] <- 0
data_SL_nocancer2 <- data_SL_nocancer[!(rowSums(data_SL_nocancer)==0),!(colSums(data_SL_nocancer)==0)]

SL_nocancer2.dist1 <- vegdist(as.data.frame(t(data_SL_nocancer2)), "jaccard",na.rm = T, binary = T)
SL_nocancer2.dist <- as.data.frame(as.matrix(SL_nocancer2.dist1))
PCOA.dist_SL_nocancer2 <- pcoa(SL_nocancer2.dist)
pengi.nm<-barplot(PCOA.dist_SL_nocancer2$values$Relative_eig[1:10])
biplot.pcoa(PCOA.dist_SL_nocancer2)
dim(PCOA.dist_SL_nocancer2$vectors[,1:20])

df.SeCo3<- as.data.frame(PCOA.dist_SL_nocancer2$vectors[,1:20])
df.SeCo3$Disease <- colnames(data_SL_nocancer2)
df.SeCo3$Type <- sub("(_[^_]+)$", "",df.SeCo3$Disease)

df.SeCo3 <- ggplot(df.SeCo3, aes(x=Axis.1, y=Axis.2, color=Type, label=Disease)) + geom_point() + geom_text_repel(data=subset(df.disease, Type=="LYMP"))
df.SeCo3 

hc.SeCo3 <- hclust(SL_nocancer2.dist1, method = "ward.D")
plot(hc.SeCo3, cex = 0.8, hang = -1)

pclust.SeCo3 <- fviz_dend(hc.SeCo3, cex = 1, k=1 ,type = "phylogenic", rect = TRUE, repel=T, k_colors = "black") + scale_color_manual(values=c("black"))
pclust.SeCo3 <- pclust.SeCo3 + ggtitle("Disease relationship of patient with secondary Lymphodema without cancer (n=407)")
pclust.SeCo3
ggsave(file = "./figure/Figure4_SLnoCancer_Black.pdf", pclust.SeCo3, height =9, width = 7.5, dpi=300)

write.csv(file= "./SL_noCancer_pairwised_distance.csv", as.matrix(SL_nocancer2.dist1))


#prevalance figure
Prev <- as.data.frame(colSums(SL[,21:144])/407)
Prev$LV <- colSums(LV[,21:144])/116
Prev$LP <- colSums(LP[,21:144])/93
Prev$NM <- colSums(NM[,21:144])/106
colnames(Prev)[1] <- "SL"
colnames(Prev)

write.csv(file = "./PrevalanceMatrix.csv", Prev)


#calculate the total comorbidity used for analysis

colSums(data_ori[21:144])[colSums(data_ori[21:144]) == 0]
